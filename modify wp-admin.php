...
/*
 * Plugin Name: Modify WP-Admin.PHP
 * Plugin URI:  https://wordpress.org/plugins/modify-wp-login/
 * Description: Do You Want To Secure Your WordPress Website. Modify WP-Admin.php Pluign Will Allow User to Change WP-Admin.php And WP-Login.php Fast & Easy.
 * Version:     1.2.10
 * Author:      Neel G Patel
 * Author URI:  https://www.wordpress.org/
 * Text Domain: modify-wp-login
 * Domain Path:
 * Network:
 * License:     GPL-2.0+
 ...
 */
 ...
 if ( defined( 'ABSPATH' ) && ! class_exists( 'Modify_WP_Login' ) ) {

 	class Modify_WP_Login {
 		...
 		public function __construct() {
 			...
 			register_uninstall_hook( $this->basename(), array( 'Modify_WP_Login', 'uninstall' ) );
 		...
 		public function wpmu_options() {
 			echo (
 				'<h3>' .
 					_x( 'Modify wp-login.php', 'Text string for settings page', 'Modify-wp-login' ) .
 				'</h3>' .
 				'<p>' .
 					__( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'Modify-wp-login' ) .
 				'</p>' .
 				'<table class="form-table">' .
 					'<tr valign="top">' .
 						'<th scope="row">' .
 							__( 'Networkwide default', 'Modify-wp-login' ) .
 						'</th>' .
 						'<td>' .
 		...
 		public function admin_init() {
 			...
 			add_settings_field(
 				'rwl-page',
 				'<label for="rwl-page">' . __( 'Login url', 'Modify-wp-login' ) . '</label>',
 		...
 		public function rwl_section_desc() {
 			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
 				echo (
 					'<p>' .
 						sprintf(
 							__( 'To set a networkwide default, go to %s.', 'Modify-wp-login' ),
 							'<a href="' . esc_url( network_admin_url( 'settings.php#rwl-page-input' ) ) . '">' .
 								__( 'Network Settings', 'Modify-wp-login' ) .
 							'</a>'
 						) .
 					'</p>'
 				);
 		...
 		public function admin_notices() {
 			...
 			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
 				echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'Modify-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
 			}
 		}
 		...
 		public function plugins_loaded() {
 			...
 			load_plugin_textdomain( 'Modify-wp-login' );
 			...
 			if (
 				...
 				wp_die( __( 'This feature is not enabled.', 'Modify-wp-login' ), '', array( 'response' => 403 ) );
 			}
 		...
 		public function wp_loaded() {
 			...
 			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
 				wp_die( __( 'You must log in to access the admin area.', 'Modify-wp-login' ), '', array( 'response' => 403 ) );
 			}
